...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  try {
    ...
    const listing = await getCategoryListing(slug, { page, pageSize: 24 });
    ...
    return buildMetadata({
      ...
      path: page > 1 ? `/kategorije/${slug}?page=${page}` : `/kategorije/${slug}`,
      seo: listing.seo,
      type: 'website',
    });
  } catch {
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  try {
    ...
    const resolvedSearchParams = (await searchParams) || {};
    const page = getRequestedPage(resolvedSearchParams.page);
    const data = await getCategoryListing(slug, { page, pageSize: 24 });
              ...
              {pageCount > 1 && (
                <div className="mt-8 flex flex-wrap justify-center gap-2">
                  {Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => (
                    <Link
                      key={pageNumber}
                      href={pageNumber === 1 ? `/kategorije/${slug}` : `/kategorije/${slug}?page=${pageNumber}`}
